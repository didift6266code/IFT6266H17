...
class DISCRIMINATOR_NET():
    ...
    def __init__ (self,batch_size=4, num_filters=[2,4,8,16,32,32,32,32,1]):
            ...
            self.L1=Conv_discr_layer(num_filters=num_filters[0],filter_channels=3)
            self.L1_pool=Conv_pool_discr_layer(num_filters=num_filters[1],filter_channels=num_filters[0])
            self.L2_pool=Conv_pool_discr_layer(num_filters=num_filters[3],filter_channels=num_filters[1])
            self.L3_pool=Conv_pool_discr_layer(num_filters=num_filters[5],filter_channels=num_filters[3])
            self.L4_pool=Conv_pool_discr_layer(num_filters=num_filters[7],filter_channels=num_filters[5])
            self.L5_OUT=Conv_discr_out_layer(num_filters=num_filters[8],filter_channels=num_filters[7]) 
